...
- 研究团队 API 使用 `research-teams` 和 `research-team-roles`。会议、自选列表、唤醒计划、摄取消息过滤器和消息订阅使用显式的 `researchTeamId` 或 `teamIds` 字段，而不是隐式全局团队。`research-teams` 额外暴露 `assetClass`，取值为 `a_share`、`prediction_market` 或 `mixed`；`prediction_market` 团队允许 `paperAccountId=null`，`a_share` 和 `mixed` 仍要求有效模拟盘账户。保存预测市场团队角色、复制其他团队角色或将已有团队切换为 `prediction_market` 时，服务端只保留 `web.search`、`meeting.*`、`prediction.*` 工具和预测市场研究白名单技能，并为自定义/复制角色 prompt 加入预测市场边界，前端角色编辑器也按同一规则过滤可选能力。设置向导的预测市场默认项会修复旧版“默认预测市场投研团队”的资产类型、模拟盘账户和默认角色定义，但保留角色已配置的 provider/model/enabled。
- 预测市场 API 使用独立资源类型：`prediction-events`、`prediction-markets`、`prediction-market-quotes`、`prediction-market-matches` 和 `prediction-watchlist-items`。v1 仅接入 Polymarket 公共数据，不保存钱包、API key、订单、仓位、充值或提现信息；预测市场不会写入 A 股 `market_symbols`。Polymarket Gamma/CLOB 请求复用 Market 代理模块，前端与设置向导把该代理语义展示为“行情/预测市场”。`GET /prediction-markets/search` 会把 Polymarket event/events/market/markets URL（包括无协议裸域名 URL）或带分享 fragment 的 slug 归一化为小写 Gamma slug，URL/slug 优先通过 Gamma 精确 slug 接口持久化完整候选，普通关键词使用完整 `/public-search` payload；响应 meta 暴露 `query`、`normalizedQuery`，当 provider 失败但本地缓存命中时暴露 `providerWarning`，provider 失败且无本地命中时返回 `prediction-market-provider-unavailable`；前端在 URL 被归一化、显示缓存结果、provider 失败且无本地缓存命中时分别展示 info/warning/error 提示，错误提示会说明已尝试的 slug 与“外部搜索失败且本地缓存无命中”。本地缓存兜底会优先返回 exact market slug/external id/condition id，其次返回 exact event slug/external event id，再按部分匹配、活跃度、关闭状态和 volume 排序。`POST /prediction-markets/sync` 同步活跃市场，`GET /prediction-matches` 和 `POST /prediction-matches/{matchId}/review` 用于新闻到预测市场匹配治理，OpenAPI 对 match/watchlist attributes 显式声明 `marketId`、`status`、`score`、`market`、`sourceMeetingId`、`sourceMeetingEventId` 和 `sourceRoleKey` 等字段，不使用弱 `additionalProperties` 契约；候选匹配只会在消息订阅绑定 `prediction_market` 或 `mixed` 团队且消息未被过滤为 ignore 时保存，纯 A 股订阅不会生成预测市场 match；预测市场团队过滤结果会强制清空 `related_symbols`，避免模型误填 A 股代码污染消息摘要；`GET/POST /prediction-watchlist` 仅允许明确的 `assetClass=prediction_market|mixed` 团队读取或关注已存在预测市场，不提供空团队或跨团队全局列表。会议 recap 写入 `prediction_watchlist_actions` 时，关注项会保存来源会议、来源 recap 事件和来源角色；会议详情的结构化证据可跳到预测市场页，预测市场关注列表可跳回来源会议，手工关注更新不会清空已有来源。摄取消息额外暴露只读 `relatedPredictionMarkets` 和 `predictionMarketMatchStatus`；会议创建可传 `predictionMarketIds`，`prediction_market` 团队固定按预测市场会议运行，`mixed` 团队在本次会议带有预测市场 ID 时也按预测市场会议运行；这类消息触发建会不会从 filter result、消息聚合字段或正文兜底继承 A 股 `related_symbols`，payload 中 `related_symbols` 保持空数组，上下文以 `prediction_market_ids` 和匹配证据为准；会议工具可通过 `prediction.search_markets`、`prediction.market_snapshot`、`prediction.orderbook`、`prediction.price_history` 和 `prediction.related_matches` 拉取证据，其中 `prediction.search_markets` 会复用应用层搜索并返回已落库的本地 `market_id`，已归属 event 的候选、snapshot/orderbook/price history 和 related match market 明细会暴露本地 `event_id`、`external_event_id`、`event_slug` 和 `event_title`，并在工具参数中保留 `original_query` 与 `normalized_query` 便于审计和后续工具串联；snapshot/orderbook/price history 会输出 `outcome_tokens`，盘口和历史价格还会标注当前 token 的 `outcome`、`outcome_index` 和 `outcome_price`，避免 Yes/No token 被误读。预测市场会议中的 `web.search` 使用 global/US 新闻搜索参数，且不会把历史 context 中的 A 股 `related_symbols` 拼入搜索 query；prediction meeting 的角色上下文会过滤掉残留的非 `web.search`/`meeting.*`/`prediction.*` 工具说明；托管 prediction meeting 的角色发言也会在写入讨论上下文前检查 A 股/证券代码/模拟盘/买卖/仓位/market.* 或 paper.* 污染，模型会先重写，仍违规时仅记录 `prediction_market_role_analysis_blocked` 的干净占位发言；recap 只允许观察型 time/event wake plan，拒绝 indicator wake plan 以及 `code`、`symbol`、`ticker`、`related_symbols`、`symbols`、`codes` 等 A 股触发字段；recap 文本字段也会拒绝 A 股、证券代码、模拟盘、买卖/仓位/下单或 A 股行情/模拟盘工具引用，托管会议会要求模型重写，旧式 recap 会写入 `prediction_market_recap_text_blocked` 并拒绝应用为最终结论；当前会议实时价格接入优先使用 Polymarket Market WebSocket 短采样并写入 tool result，失败时降级 CLOB REST 盘口；WebSocket runner 状态在 provider health 中明示。
- `PredictionMarketAttributes` 除 `eventId` 外还暴露 `externalEventId`、`eventSlug` 和 `eventTitle`；搜索结果、市场详情、match/watchlist 嵌套 market 都应携带这些可读 event 身份字段，前端预测市场页用它们展示 event 归属，而不是只展示本地数字 ID。
- 预测市场自动匹配的 `scoreBreakdown` 暴露 `overlap_terms`、overlap/news/market term 计数和 event 身份字段，`candidateSnapshot` 使用稳定 snake_case market/event 身份字段；自动 `linked` 除分数阈值外还要求至少 2 个不同关键词重合，单词重合只能进入人工复核。
...
- 设置向导 API 使用 `setup-readinesses` 和 `setup-action-results`。`GET /setup/readiness` 汇总管理员、AI、团队、预测市场、模拟盘风控、行情、消息源、代理、日志和队列状态；`POST /setup/actions/{key}` 只执行幂等初始化动作，不替代各领域的完整配置页面。`ensure-prediction-market-defaults` 会幂等创建或修复默认预测市场投研团队和默认预测市场消息过滤器，不会创建预测市场模拟盘。
