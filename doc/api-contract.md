...
- 设置向导 API 使用 `setup-readinesses` 和 `setup-action-results`。`GET /setup/readiness` 汇总管理员、AI、团队、模拟盘风控、行情、消息源、代理、日志和队列状态；`POST /setup/actions/{key}` 只执行幂等初始化动作，不替代各领域的完整配置页面。
...
- `GET /ops/provider-health` 的 AI 诊断暴露 provider 可用性、近 24 小时/累计 token 用量、provider/model 聚合、成本金额、成本来源、预算状态、预算使用率、启用模型价格覆盖率、缺价格模型样本、缺价格调用、估算调用和模型调用延迟均值/P95；meeting runtime 诊断暴露近 24 小时会议完成/失败/运行数量、排队等待、运行时长 P95 和事件类型计数；market/messaging 诊断暴露行情最新报价年龄、日线年龄、简易健康分、消息过滤状态分布、采集错误、到期订阅和去重策略说明。
