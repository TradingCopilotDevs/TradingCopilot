...
- 高增长列表必须使用基于游标的分页，包括 `page[limit]`、`page[cursor]`、`links.next` 和 `meta.nextCursor`；客户端必须把游标视为不透明的透传值。
...
- 使用游标分页的易变列表，在最新模式下只能刷新最新页。加载更早记录会进入历史模式，显示可见提示，并暂停自动刷新，直到用户返回最新模式。
- 视图文件不得直接创建原始 `setInterval` 定时器或 `EventSource` 流；生命周期清理由共享组合式函数负责。

## 日志

- 日志视图必须使用 `useAutoRefresh`，提供表格和移动端卡片兜底视图，并使用详情抽屉展示结构化字段。
...
- 提供方配置区域只展示有可编辑凭据的提供方。对于契约上有意无需凭据的提供方，不要渲染占位凭据标签页。
